...
function NeetPG() {
  ...
  // NEET PG Course ID found from API
  const COURSE_ID = "1420d4d3-6d23-46f9-9182-726c808f5be4";
    ...
    <div className="min-h-screen bg-gray-50 pt-32 pb-20">
      ...
      <section className="relative mb-16">
        <div className="relative h-75 md:h-100 overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-linear-to-r from-emerald-900/95 to-emerald-800/70 flex flex-col justify-center px-6 md:px-20">
            <div className="animate-fade-in-up">
              <span className="bg-emerald-500/20 text-emerald-100 border border-emerald-400/30 px-4 py-1.5 rounded-full text-sm font-semibold uppercase tracking-wider mb-4 inline-block backdrop-blur-sm">
            ...
            <p className="text-emerald-100 text-lg md:text-xl max-w-2xl animate-fade-in-up delay-100 leading-relaxed">
              {course?.description || 'Advanced medical concepts for postgraduate entrance exams. Expert-curated content for success.'}
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-10 flex flex-wrap gap-8 items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Advanced Modules</h2>
            <p className="text-gray-500 text-sm">Targeted learning for PG success</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.map((subject) => (
              <div
                ...
                className={`group bg-white rounded-2xl p-6 hover:shadow-xl transition-all duration-300 cursor-pointer border border-gray-100 hover:border-emerald-200 relative overflow-hidden ${!hasAccess ? 'opacity-75' : ''
                  }`}
              >
                <div className="absolute top-0 right-0 w-24 h-24 bg-linear-to-br from-emerald-50 to-transparent rounded-bl-full opacity-50 group-hover:scale-110 transition-transform -mr-4 -mt-4"></div>
      ...
      <UpgradePrompt
        ...
        user={user}
        courseId={COURSE_ID}
        courseName={course?.name || 'NEET PG'}
      />
    </div>
  );
}
